...
| Branch(ブランチ)ビュー | 全画面マージビュー |
| --- | --- |
| ![Branch ビュー —— ブランチの first-parent 履歴、マージをその場で展開](screenshots/branch-view.png) | ![全画面マージビュー —— マージが取り込んだコミット、パンくず付き](screenshots/merge-view.png) |
...
- **Branch(ブランチ)** —— ブランチの first-parent 履歴(`git log --first-parent`)。
  ...
  - **ダブルクリック**で**全画面マージビュー**を開きます —— 取り込んだコミットの
...
src/
  ...
  MergeView.tsx         全画面マージ overlay(ドリルスタック + パンくず、仮想化)
...
- **コミット数は背景で埋まる。** Git はマージごとの問い合わせなしに内包コミット数を
  ...
  届くまでその欄は淡色表示です。
- **リストはすべて仮想化。** 長いリスト(ブランチ履歴と全画面マージビュー)はすべて
