...
配布される `.dmg` は **ad-hoc 署名で、公証(notarization)はされていません**。
インターネットからダウンロードされるため macOS が隔離属性を付与し、初回起動が
ブロックされます(「開発元を検証できません」)。インストール後に一度だけ、次の
コマンドで隔離属性を解除してください:
...
または、アプリを右クリック →**開く**、あるいは**システム設定 → プライバシーと
セキュリティ →「このまま開く」**。ダウンロードごとに一度だけの操作です。
...
DeltaScope は **GNU Affero General Public License v3.0**(AGPL-3.0)でライセンスされています。
自由に使用・改変・配布できますが、派生物（ネットワーク経由で提供されるソフトウェアを含む）は
同一のライセンスで公開し、ソースコードを提供する必要があります。
