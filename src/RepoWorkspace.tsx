...
export interface RepoWorkspaceProps {
  repoPath: string; // App-owned; changing it reloads this workspace
  initialViewBranch?: string; // persisted branch to restore on first load
  hidden: boolean; // inactive tab — kept mounted, visually hidden
  onViewBranchChange: (branch: string) => void; // report branch for label + persistence
  onRepoChange: (repoPath: string) => void; // folder button swapped this tab's repo
}
...
export default function RepoWorkspace({
  ...
  // persisted branch to restore on first load only (consumed once)
  const pendingInitialBranch = useRef(initialViewBranch ?? "");
  ...
  // (Re)open whenever the repoPath prop changes: resolve branches + the branch
  // to show (persisted on first load, else default), then load it. Reports the
  // chosen branch upward so App can update the tab label + persist it.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      setLoadError(null);
      let def = "";
      try {
        def = (await defaultBranch(repoPath)).trim();
      } catch {
        /* detached / bare — fall through */
      }
      let brs: string[];
      try {
        brs = await listBranches(repoPath);
      } catch (e) {
        if (cancelled) return;
        setBranches([]);
        setData(EMPTY);
      ...
      if (cancelled) return;
      setBranches(brs);
      const want = pendingInitialBranch.current;
      pendingInitialBranch.current = ""; // restore only on the first load
      const b = (
        want && brs.includes(want) ? want : def || brs[0] || ""
      ).trim();
      setViewBranch(b);
      onViewBranchChange(b);
      if (b) await loadBranch(repoPath, b);
      else setData(EMPTY);
    })();
    return () => {
      cancelled = true;
    };
    // Intentionally only re-run when the repo changes; loadBranch/onViewBranchChange
    // are stable callbacks and are excluded to avoid spurious reloads.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [repoPath]);

  // Folder button: swap this tab's repo. App updates repoPath → effect reloads.
    ...
    <div className="workspace" style={{ display: hidden ? "none" : "flex" }}>
      ...
      <div className="toolbar">
        <button className="btn" onClick={onPick}>
          ...
          <span className="path">{tildify(repoPath)}</span>
          ...
          <div className="list-region">
            <div className="list" ref={listRef}>
              {loadError ? (
                <div className="empty">
                  ...
                  <span className="msg">
                    {t("repo.lastOpenFailed", { path: tildify(repoPath) })}
                  </span>
                    ...
                    return (
                      ...
                      >
                        <div
                          ...
                          onClick={() =>
                            c.isMerge ? onToggle(c) : openCommitDiff(c)
                          }
                          onDoubleClick={() => {
            ...
            {mergeStack.length > 0 && (
              <MergeView
                ...
                loading={
                  !!loadingCommits[mergeStack[mergeStack.length - 1].fullHash]
                }
                onBack={mergeBack}
